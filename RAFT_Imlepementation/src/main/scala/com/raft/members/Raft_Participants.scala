...
class Raft_Participants(candidateID: String) extends Actor with ActorLogging with Timers {
  ...
  override def receive: Receive = {
    ...
    case result: RESULT_APPEND_ENTRIES =>
      ...
      if (result.decision && myState != FOLLOWER) {
        ...
        if (votedNodes.size >= (1 + (activeNodes.size) / 2)) {
          ...
          //skipCommit wil be true when a candidate just became the leader and has data on its wall
          // which means the previous leader that requested consensus has crashed
          if (myWall.size > 0 && !skipCommit) {
            ...
            if (commitStatus) {
              ...
              println(s"\n\n  ============ $data is committed successfully =============== ")
            }
            else {
              data.sender ! "TRY AGAIN!"

              println(s"\n\n  ============ OOPS $data is could not committed  =============== ")
            }
            myWall.clear()
    ...
    case logInconsistency: APPEND_ENTRIES_LOG_Inconsistency =>
      ...
      if (myState == LEADER) {
        ...
        println(s"\n\n =============== APPEND_ENTRIES_LOG_Inconsistency ${sender().path.name} : START")
    ...
    case MemberUp(member) =>
      ...
      if (activeNodes.size > 2 && !isActive) {
  ...
  def handleAppendEntryRequest(appendEntryFromLeader: APPEND_ENTRIES) = {
    ...
    if (leaderLogEntries.size > 0 || !myWall.isEmpty)
      DEBUG = true
    ...
    logger(s" myLoglength $myLoglength lastCommitedEntry=$lastCommitedEntry mycurrentTerm= ${currentTerm} myWall=${myWall}")
    ...
    if (term_Leader >= currentTerm) {
      ...
      if (currentLeader != sender()) {
          ...
          println(s"Received Heartbeat from ${sender.path.name}")
      ...
      if(leaderLogEntries.size == 0 && prevLeaderLogEntry_Leader != emptyLogEntry && !myWall.isEmpty){
        if(prevLeaderLogEntry_Leader.command ==myWall(0).clientCommand){
          ...
          println(s" 1. Mywall ${myWall}")
          myWall.clear()
          println(s" 2. Mywall ${myWall}")
          persistData()
      ...
      if (!alreadyReplied) {
        ...
        if (proceedWithCommit) {
          ...
          if (leaderCommitIndex_Leader > lastApplied && leaderLogEntries.size > 0) {
            ...
            deletEntriesIfExists(deleteAfterIndex = commitIndex + 1,false )
            if (commitChanges) {
              persistData()
            }
          }
    ...
    if (!alreadyReplied) {
      logger(s"\n\n @@@@@@@@@@@@ Sending HEartbeat response as $success - my term ${currentTerm} commited_Entries.size = ${commited_Entries.size} leaderLogEntries.size= ${leaderLogEntries.size} \n\n ")
      if(success && leaderLogEntries.size != 0)
      {
        myWall += Command_Message(leaderLogEntries(0).sender,leaderLogEntries(0).command)
      }

      if(lastCommitedEntry == prevLeaderLogEntry_Leader && !myWall.isEmpty){

        println("Clearing my wall as my last commit entry is same as leader's prev commited entry...")
        myWall.clear();
  ...
  def sendAliveMessage(postCommitLog: Boolean = false) = {
    if (myState == LEADER) {
      ...
      if (pendingCommands.size > 0 && myWall.size == 0) {
        ...
        println(s" **** posting data on my wall  myWall=${myWall}")
      ...
      if(iWaitPrintCount  % 5 ==0) {
        iWaitPrintCount=5
        println(s"\n sending Heartbeat for term $currentTerm prevLogEntry = $prevLogEntry logSize = $logSize ")
      }
      iWaitPrintCount -= 1
  ...
  def createTimerActor(START_SENDING_TIMER: Boolean = true): Unit = {
    ...
    timerActor = context.actorOf(Props(classOf[RAFT_TimerHandler], myState), name = "timer" + iTimerActorCount.incrementAndGet())
...
//      readFromStateMachine()
  ...
  var iTimerActorCount: AtomicInteger = new AtomicInteger(0)
