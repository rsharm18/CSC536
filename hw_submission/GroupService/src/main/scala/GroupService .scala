...
class GroupServer (val myNodeID: Int, val numNodes: Int, storeServers: Seq[ActorRef], burstSize: Int) extends Actor {
  ...
  def receive() = {
      case Prime() =>
        ...
        if(WRITETOFILE==true) {
          println(s"\n [$myNodeID] WRITETOFILE flag is true. This might lead to timeout. Please either set WRITETOFILE=false in GroupService or adjust opsPerNode/burst size to avoid timeouts. Please check project root directory for generated log ")
        }
        append=true;
   ...
   // println(s" ${myNodeID} multicasting to  ${parentGroupList.length} groups : Start")
    if(parentGroupList.length>0){
      ...
      var groupMembers = getGroupMembers(grpID)
        ...
        {
	  //multicast message to all members including myslef
